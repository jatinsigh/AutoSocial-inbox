...
export default function Home() {
  ...
  // Quick actions state (only used when authenticated)
  ...
  //   });
  //   const t = setInterval(() => {
  //     sb.auth.getSession().then(({ data }) => setIsAuthed(!!data.session));
  //   }, 15000);
...
useEffect(() => {
  ...
  // --- Quick Actions handlers (auth only) ---
  async function sendTemplate() {
    ...
    try {
      const res = await fetch("/api/wa/send", {
        ...
        body: JSON.stringify({ to, kind: "template", template: { name: tplName, language: { code: tplLang } } }),
      }).then((r) => r.json());
  ...
  return (
    <main className="container">
      ...
      <section className="grid" style={{ marginTop: 18 }}>
        <div className="card">
          <div className="card-body">
            <h1 className="card-title" style={{ fontSize: 22, marginBottom: 6 }}>Collect payments on WhatsApp</h1>
        ...
        <div className="card">
          <div className="card-body">
            ...
            <div className="grid grid-2">
              ...
              <Pill label="Public URL" ok={!!status?.env?.NEXT_PUBLIC_APP_URL} note={status?.env?.NEXT_PUBLIC_APP_URL || undefined} />
            </div>
      ...
      {/* If not logged in, nudge to login instead of showing actions */}
      {!isAuthed && (
        <section className="grid" style={{ marginTop: 16 }}>
          <div className="card">
            <div className="card-body">
              ...
              <p className="muted" style={{ marginTop: 0 }}>
                Log in to send templates, texts, and payment links from the dashboard.
              </p>
